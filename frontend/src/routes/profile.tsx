...
function loadNotesOverride(): Record<string, string> {
  try {
...
function saveNotesOverride(coachId: string, notes: string) {
  const all = loadNotesOverride();
  all[coachId] = notes;
...
function ProfilePage() {
  ...
  const [notes, setNotes] = useState<string>(
    coachProfile ? (loadNotesOverride()[coachProfile.id] ?? coachProfile.notes ?? "") : ""
  );
  ...
  useEffect(() => {
    setNotes(coachProfile ? (loadNotesOverride()[coachProfile.id] ?? coachProfile.notes ?? "") : "");
  }, [coachProfile?.id]);
  ...
  const handleSave = () => {
    if (coachProfile) {
      ...
      saveNotesOverride(coachProfile.id, notes);
    }
    setEditing(false);
  };
    ...
    <AppShell title="Профиль" subtitle="Личная информация">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <Card className="p-6 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-4">
            <span className="text-xs font-semibold text-muted-foreground">Дополнительно</span>
            {editing ? (
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Дополнительная информация о тренере..."
                className="mt-1.5 min-h-[100px] w-full rounded-lg border border-border bg-background p-3 text-sm text-secondary placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y"
              />
            ) : (
